...
fn solve(msh: &CatmarkMesh<f64, 2>, u: impl Fn(Point2<f64>) -> Vector1<f64>, u_grad: impl Fn(Point2<f64>) -> Vector2<f64>, f: impl Fn(Point2<f64>) -> Vector1<f64>) -> (usize, f64, f64, f64, f64) {
    // Define space
    ...
    let err_fn = |elem: &&CatmarkPatchNodes, x: (f64, f64)| {
        ...
        (u_grad(p) - uh.eval_grad_on_elem(elem, x)).norm()
    };
