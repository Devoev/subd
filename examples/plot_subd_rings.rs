...
fn main() {
    ...
    let nodes = quad.nodes_elem(&msh.cells[0].to_element(&msh.coords)).collect_vec();
    dbg!(&nodes);
    ...
    // Plot
    plot_fn_msh(&msh, &|cell, uv| f(cell.to_element(&msh.coords).geo_map().eval(uv)), 10, |elem, num| {
        (lin_space(0.0..=1.0, num).collect(), lin_space(0.0..=1.0, num).collect())
    }).show();

    // Write
    let z = msh.coords.iter().map(|&p| f(p));
    write_connectivity(msh.cell_iter().copied(), &mut File::create("examples/surf_conn.dat").unwrap()).unwrap();
