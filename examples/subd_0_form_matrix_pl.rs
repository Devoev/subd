...
fn main() {
    ...
    let space_coarse = Space::<f64, _>::new(PlBasisQuad(&msh_coarse));
    ...
    // Define fine space
    let space_fine = Space::<f64, _>::new(PlBasisQuad(&msh));
    ...
    let elem_fine = &msh.cells[0];
    let elem_coarse = &msh_coarse.cells[0];
    ...
    let mass_matrix_dec = Hodge::new(&msh, &space_fine).assemble(&quad);
