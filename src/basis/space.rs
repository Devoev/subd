...
type LocalSpace<T, B, const D: usize> = Space<T, <B as LocalBasis<T>>::ElemBasis, D>;

/// The local space [`LocalSpace`] paired with the indices [`LocalBasis::global_indices`]
/// of nonzero basis functions.
type LocalSpaceWithIdx<T, B, const D: usize> = (LocalSpace<T, B, D>, <B as LocalBasis<T>>::GlobalIndices);

impl <T: RealField, B: LocalBasis<T>, const D: usize> Space<T, B, D>
    where DefaultAllocator: EvalBasisAllocator<B::ElemBasis>
{
    /// Returns this space restricted to the local element `elem`.
    pub fn local_space(&self, elem: &B::Elem) -> LocalSpace<T, B, D> {
        Space::new(self.basis.elem_basis(elem))
    }

    /// Returns this space restricted to the local element `elem`
    /// as well the indices corresponding to the global numbering of local basis functions.
    pub fn local_space_with_idx(&self, elem: &B::Elem) -> LocalSpaceWithIdx<T, B, D> {
...
type EvalLocal<T, B> = OMatrix<T, <B as Basis>::NumComponents, <<B as LocalBasis<T>>::ElemBasis as Basis>::NumBasis>;

/// The basis matrix [`EvalLocal`] paired with the indices [`LocalBasis::global_indices`]
/// of nonzero basis functions.
type EvalLocalWithIdx<T, B> = (EvalLocal<T, B>, <B as LocalBasis<T>>::GlobalIndices);

impl <T: RealField, B: LocalBasis<T>, const D: usize> Space<T, B, D>
    where DefaultAllocator: EvalBasisAllocator<B::ElemBasis>
{
    /// Evaluates only the local basis functions on the given `elem` at the parametric point `x`.
    pub fn eval_on_elem(&self, elem: &B::Elem, x: B::Coord<T>) -> EvalLocal<T, B> {
    ...
    pub fn eval_on_elem_with_idx(&self, elem: &B::Elem, x: B::Coord<T>) -> EvalLocalWithIdx<T, B> {
        let local_basis = self.basis.elem_basis(elem);
    ...
    pub fn populate_global_on_elem(&self, global: &mut OMatrix<T, B::NumComponents, B::NumBasis>, elem: &B::Elem, x: B::Coord<T>) {
...
type EvalGradLocal<T, B, const D: usize> = OMatrix<T, Const<D>, <<B as LocalBasis<T>>::ElemBasis as Basis>::NumBasis>;

/// The gradient matrix [`EvalGradLocal`] paired with the indices [`LocalBasis::global_indices`]
/// of nonzero basis functions.
type EvalGradLocalWithIdx<T, B, const D: usize> = (EvalGradLocal<T, B, D>, <B as LocalBasis<T>>::GlobalIndices);
          ...
          DefaultAllocator: EvalGradAllocator<B::ElemBasis, D>
{
    ...
    pub fn eval_grad_on_elem(&self, elem: &B::Elem, x: B::Coord<T>) -> EvalGradLocal<T, B, D> {
        let local_basis = self.basis.elem_basis(elem);
    ...
    pub fn eval_grad_on_elem_with_idx(&self, elem: &B::Elem, x: B::Coord<T>) -> EvalGradLocalWithIdx<T, B, D> {
        let local_basis = self.basis.elem_basis(elem);
    ...
    pub fn populate_grad_global_on_elem(&self, global: &mut OMatrix<T, Const<D>, B::NumBasis>, elem: &B::Elem, x: B::Coord<T>) {
