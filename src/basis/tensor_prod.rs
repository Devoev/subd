...
// todo: these global functions can be removed, once GlobalBasis requires Basis to be a supertrait
impl<T: RealField, B: GlobalBasis<T, T, 1, 1>, const D: usize> MultiProd<T, B, D> {
...
impl<T: RealField, B: LocalBasis<T, T, 1, 1, Index = usize>, const D: usize> MultiProd<T, B, D>
    where B::GlobalIndices: Clone,
{
    ...
    fn global_indices_multi_prod(&self) -> impl Iterator<Item = [usize; D]> + Clone {
        self.bases.iter()
...
impl <T: RealField, B: LocalBasis<T, T, 1, 1, Index = usize>, const D: usize> LocalBasis<T, [T; D], D, 1> for MultiProd<T, B, D>
    where B::GlobalIndices: Clone
{
    type Index = [usize; D];
    type GlobalIndices = impl Iterator<Item = [usize; D]> + Clone;
