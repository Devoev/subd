...
pub trait Quadrature<T: RealField + Sum, E, const D: usize> {
    ...
    fn integrate_elem(&self, elem: &E, f: impl IntoIterator<Item = T>) -> T {
        integrate_with_weights(self.weights_elem(elem), f)
    }

    /// Evaluates the function `f` on every [quadrature node][Self::nodes_elem]
    /// of the given `elem`.
    fn eval_fn_elem(&self, elem: &E, f: impl Fn(Self::Node) -> T) -> impl Iterator<Item = T> {
    ...
    fn integrate_fn_elem(&self, elem: &E, f: impl Fn(Self::Node) -> T) -> T {
