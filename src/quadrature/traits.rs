...
/// Quadrature rule on a reference domain.
pub trait Quadrature<T> {
    ...
    fn integrate_fn_ref(&self, f: impl Fn(Self::Node) -> T) -> T {
        self.integrate(self.eval_fn_ref(f))
    }
}
...
pub trait ElementQuadrature<T: RealField, const D: usize>: Quadrature<T> {
    ...
    fn nodes_elem(&self, elem: &Self::Elem) -> impl Iterator<Item = Point<T, D>> {
        ...
        self.nodes().map(move |n| geo_map.eval(n))
    }
    ...
    fn integrate_fn_elem(&self, elem: &Self::Elem, f: impl Fn(Point<T, D>) -> T) -> T {
        self.integrate(self.eval_fn_elem(elem, f))
    }
}