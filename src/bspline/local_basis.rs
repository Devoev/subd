...
impl <'a, T: RealField> LocalBsplineBasis<'a, T> {
    /// Constructs a new [`LocalBsplineBasis`] from the given `global_basis` and `span`.
    pub fn new(knots: &'a KnotVec<T>, degree: usize, span: KnotSpan) -> Self {
        Self { knots, degree, span }
    }
}

impl <'a, T: RealField + Copy> LocalBsplineBasis<'a, T> {
