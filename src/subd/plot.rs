...
pub fn plot_patch_fn_pullback(b: impl Fn(f64, f64) -> f64, num: usize) -> Plot {
    let mut plot = Plot::new();
    let min = 1e-5;
...
pub fn plot_surf_fn(msh: &QuadMesh<f64>, f: impl Fn(Point2<f64>) -> f64 + Clone, num: usize) -> Plot {
    ...
    for patch in msh.patches() {
        let plot_patch = plot_patch_fn(patch, f.clone(), num);
...
pub fn plot_surf(msh: &QuadMesh<f64>, num: usize) -> Plot {
    plot_surf_fn(msh, |plot| plot.coords.norm(), num)
}
