...
use nalgebra::{Const, DefaultAllocator, DimMin, Dyn, OVector, Point, RealField};
use std::iter::{zip, Product, Sum};

/// L2-norm on a discrete function space.
pub struct L2Norm<'a, T, X, M, B, const D: usize> {
...
impl<'a, T, X, M, B, const D: usize> L2Norm<'a, T, X, M, B, D> {
    ...
    pub fn norm<U, Q>(&self, u: U, quad: PullbackQuad<T, X, M::GeoElem, Q, D>) -> T
    ...
    /// using the quadrature rule `quad`.
    pub fn error<U, Q>(&self, uh: &LinCombination<T, X, B, D>, u: &U, quad: PullbackQuad<T, X, M::GeoElem, Q, D>) -> T
    where T: RealField + Copy + Product<T> + Sum<T>,
          X: Dimensioned<T, D> + Copy,
          M: Mesh<'a, T, X, D, D, Elem = B::Elem>,
          M::GeoElem: Cell<T, X, D, D>,
